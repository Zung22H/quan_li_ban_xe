...
class CarAdmin(admin.ModelAdmin):
 ...
 # excel function
  ...
  def get_urls(self):
    urls = super().get_urls()
    product_url = [path('car-report/', self.download_product_data), ]
    return product_url + urls

  def download_product_data(self, request):
    all_product_values = Car.objects.all()
    product_data = {"Id": [], "Name": [], "Price": [], "Quantity": []}
    for car in all_product_values:
      product_data['Id'].append(car.id)
      product_data['Name'].append(car.nameCar)
      product_data['Price'].append(car.price)
      product_data['Quantity'].append(car.quantity)
